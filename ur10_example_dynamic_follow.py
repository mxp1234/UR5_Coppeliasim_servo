...
def get_sensor_data(clientID, sensor_handles):
    ...
    if valid_normals:
        ...
        last_valid_normals  = average_normal
    else:
        average_normal = last_valid_normals  # 如果没有有效的法向量，则使用默认值
...
def get_force_handle(clientID):
    ...
    _,handle2 = sim.simxGetObjectHandle(clientID,'Force_sensor#0',sim.simx_opmode_blocking)
    _,handle3 = sim.simxGetObjectHandle(clientID,'Force_sensor#1',sim.simx_opmode_blocking)
    _,handle4 = sim.simxGetObjectHandle(clientID,'Force_sensor#2',sim.simx_opmode_blocking)
    _,handle5 = sim.simxGetObjectHandle(clientID,'Force_sensor#3',sim.simx_opmode_blocking)
    _,handle6 = sim.simxGetObjectHandle(clientID,'Force_sensor#4',sim.simx_opmode_blocking)
    _,handle7 = sim.simxGetObjectHandle(clientID,'Force_sensor#5',sim.simx_opmode_blocking)
    _,handle8 = sim.simxGetObjectHandle(clientID,'Force_sensor#6',sim.simx_opmode_blocking)
    _,handle9 = sim.simxGetObjectHandle(clientID,'Force_sensor#7',sim.simx_opmode_blocking)
    _,handle10 = sim.simxGetObjectHandle(clientID,'Force_sensor#8',sim.simx_opmode_blocking)
    _,handle11 = sim.simxGetObjectHandle(clientID,'Force_sensor#9',sim.simx_opmode_blocking)
    _,handle12 = sim.simxGetObjectHandle(clientID,'Force_sensor#10',sim.simx_opmode_blocking)
...
# 构建旋转矩阵
    ...
    print('位移量------------------',Vector_world*distance)
    sim.simxSetObjectPosition(clientID, targetHandle, -1, new_position, sim.simx_opmode_blocking)

def move_object(clientID,translation,rotation):
    _,object_handle = sim.simxGetObjectHandle(clientID,'Cylinder',sim.simx_opmode_blocking)
    # 获取当前位置和方向
    _, current_position = sim.simxGetObjectPosition(clientID, object_handle, -1, sim.simx_opmode_blocking)
    _, current_orientation = sim.simxGetObjectOrientation(clientID, object_handle, -1, sim.simx_opmode_blocking)
    
    # 计算新的位置和方向
    new_position = np.array(current_position) + np.array(translation)
    new_orientation = np.array(current_orientation) + np.array(rotation)

    # 设置新的位置
    sim.simxSetObjectPosition(clientID, object_handle, -1, new_position.tolist(), sim.simx_opmode_blocking)
    # 设置新的方向
    sim.simxSetObjectOrientation(clientID, object_handle, -1, new_orientation.tolist(), sim.simx_opmode_blocking)
...
def main():
    ...
    if clientID != -1:
        ...
        _, targetHandle = sim.simxGetObjectHandle(clientID, 'target', sim.simx_opmode_blocking)
        sensor_handles = []
        _, objectHandle = sim.simxGetObjectHandle(clientID, 'Cylinder', sim.simx_opmode_blocking)
        ...
        # 定义物体运动参数
        # 步骤1: 沿Y轴移动并绕Y轴旋转
        # total_distance = 0.1  # 10cm
        # total_rotation_y = np.deg2rad(45)  # 45度转为弧度
        # steps = 100
        # movement_increment_y = [0, total_distance / steps, 0]
        # rotation_increment_y = [0, total_rotation_y / steps, 0]

        # for _ in range(steps):
        #     move_object(clientID,movement_increment_y, rotation_increment_y)
        #     # 读取传感器数据并对齐机械臂
        #     sensor_distances, average_normal = get_sensor_data(clientID, sensor_handles)
        #     # if all(distance is not None for distance in sensor_distances):
        #     align_object_with_normal_using_quaternion(clientID, targetHandle, average_normal)

        # # 步骤2: 绕世界坐标系的Z轴旋转90°
        # total_rotation_z = np.deg2rad(90)  # 90度转为弧度
        # rotation_increment_z = [0, 0, total_rotation_z / steps]

        # for _ in range(steps):
        #     move_object(clientID, [0, 0, 0], rotation_increment_z)
        #     # 读取传感器数据并对齐机械臂
        sensor_distances, average_normal = get_sensor_data(clientID, sensor_handles)
        align_object_with_normal_using_quaternion(clientID, targetHandle, average_normal)
    else:
